...
def parse_header(file_path):
    ...
    """
    col_names = pd.read_csv(file_path, header=None, nrows=1, sep='\s+', engine='python').values.tolist()[0]
...
def read_outputs(file_path, fields_out=None, key=None):
    ...
    data : pandas.DataFrame
        A data frame containing a emerge output data.
    ...
    if file_path.endswith('.h5'):
        ...
        data = pd.read_csv(file_path, names=col_names, usecols=fields_out, header=None, comment='#', skiprows=1, sep='\s+')
        return data
...
def read_statistics(file_path, universe_num=0):
    ...
    if file_path.endswith('.h5'):
        ...
        stats = {'CSFRD': [('Redshift', float), ('Csfrd_observed', float), ('Sigma_observed', float)],
                 'FQ': [('Stellar_mass', float), ('Fq_observed', float), ('Sigma_observed', float), ('Mean_ScaleFactor', float), ('Fq_model', float), ('Sigma_model', float)],
                 'SMF': [('Stellar_mass', float), ('Phi_observed', float), ('Sigma_observed', float), ('Mean_ScaleFactor', float), ('Phi_model', float), ('Sigma_model', float)],
                 'SSFR': [('Redshift', float), ('Ssfr_observed', float), ('Sigma_observed', float), ('Stellar_mass', float), ('Ssfr_model', float), ('Sigma_model', float)]}
        ...
        if stat_keys is not None:
            ...
            if not all(stat_bool):
                raise KeyError('{} are not valid statistics keys!'.format(stat_keys[[not i for i in stat_bool]]))
        else:
...
def read_parameter_file(file_path):
    ...
    param_table = pd.read_csv(file_path, names=['key', 'value'], header=None, comment='%', sep='\s+', engine='python')
    params = {}
...
def read_halo_trees(file_path, file_format='EMERGE', fields_out=None):
    ...
    if file_format == 'EMERGE':
        def SKIP(fname): return fname.read(4)
        dt = np.dtype([('haloid', np.uintc),
                       ('descid', np.uintc),
                       ('upid', np.uintc),
                       ('np', np.ushort),
                       ('mmp', np.ushort),
        ...
        if fields_out is None:
            fields_out = col_names
        return pd.read_csv(file_path, names=col_names, usecols=fields_out, header=0, comment='#', sep='\s+')
    else:
        raise NotImplementedError('Only Rockstar and Emerge halo tree formats are currently supported.')
...
def write_halo_trees(file_path, tree, Ntrees, Nhalos, TreeID, file_format='EMERGE'):
    file_format = file_format.upper()
    if file_format == 'EMERGE':
        def SKIP(fname): return fname.write(struct.pack('i',4))
        dt = np.dtype([('haloid', np.uintc),
                       ('descid', np.uintc),
                       ('upid', np.uintc),
                       ('np', np.ushort),
