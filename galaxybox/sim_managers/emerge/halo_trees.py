...
class halo_trees:
    ...
    def __init__(self, trees_path, add_attrs=None, include='halos'):
        ...
        self.trees = forest
        # no more little h
        self.trees[['mvir', 'rvir', 'X_pos', 'Y_pos', 'Z_pos']] = self.trees[['mvir', 'rvir', 'X_pos', 'Y_pos', 'Z_pos']] / (self.cosmology.H0.value / 100)
        self.trees['mvir'] = np.log10(self.trees['mvir'])
        self.trees['rvir'] = self.trees['rvir']*self.trees['scale'] # we dont want radius in comoving units....
        ...
        self.trees.loc[root_mask & forest_mask, 'hostid'] = self.trees.loc[root_mask & forest_mask].index.values
        self.trees.loc[root_mask & ~forest_mask, 'hostid'] = self.trees.loc[root_mask & ~forest_mask]['upid'].values
    ...
    def find_mergers(self, enforce_positive_MR=True):
        ...
        mergers['Minor_mvir'] = possible_progs.loc[possible_progs['descid'].isin(true_desc.index)]['mvir'].copy()
        ...
        mergers[['Scale', 'Desc_mvir']] = self.trees.loc[mergers['Desc_ID'].values][['scale', 'mvir']].values

        # Find coprogenitor properties
        main_progs = possible_progs.loc[(possible_progs['mmp']==1) & possible_progs.descid.isin(mergers['Desc_ID'])]
        ...
        mergers.loc[desc_mask, 'Main_ID'] = main_progs.loc[mergers.loc[desc_mask]['Desc_ID'].values].haloid.values
        ...
        if enforce_positive_MR:
            ...
            mergers.loc[swap, ['Main_ID', 'Main_mvir', 'Minor_ID', 'Minor_mvir']] = mergers.loc[swap, ['Minor_ID', 'Minor_mvir', 'Main_ID', 'Main_mvir']].values
        # compute mass ratio
