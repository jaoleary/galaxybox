...
class Universe:
    ...
    @classmethod
    def clone(cls, universe, model_name, clone_current=False, clone_include=False, include=None):
    ...
    def add_halo_trees(self, include='halos'):
        ...
        if self.sim_type == 'EMERGE':
            ...
            print('The halo trees class is not available for simulation type ' + self.sim_type + '.')
    ...
    def make(self, clean=False, **kwargs):
        ...
        if self.sim_type == 'EMERGE':
            ...
            raise TypeError('The `make()` method is not available for simulation type ' + self.sim_type + '.')

    def run(self, **kwargs):
        """Execute `emerge` using loaded config and parameterfile."""
        if self.sim_type == 'EMERGE':
            cmd(['mpiexec', '-np', '{:d}'.format(self.num_procs), './emerge', self.param_path], path=self.emerge_dir, **kwargs)
        else:
            raise TypeError('The `run()` method is not available for simulation type ' + self.sim_type + '.')

    def update(self, params=True, config=True, flush=False, clean=False, make=False, run=False, reload=False):
