    ...
    for i, og in enumerate(vert):
        ...
        if snaparg in snap_args:
            ...
            galaxies[["X_pos", "Y_pos", "Z_pos"]] = LC.transform_position(
                pos=galaxies[["X_pos", "Y_pos", "Z_pos"]], randomize=args.randomize, box_coord=bc
            )
            if args.randomize:
            ...
            galaxies["Redshift_obs"] = (
                snap_redshift + galaxies["R_vel"] * (1 + snap_redshift) / apconst.c.to("km/s").value
            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Create mock lightcone catalogs from Emerge/GalaxyNet snapshot data. \
    ...
    parser.add_argument(
        ...
        action="store",
        default=None,
        help="Minimum redshift for the galaxy catalog.",
    )
    parser.add_argument(
        "--max_z",
        type=float,
        action="store",
        default=None,
        help="Maximum redshift for the galaxy catalog.",
    )
    parser.add_argument(
        "--np",
        type=int,
        action="store",
        default=1,
        help="Number of snapshots to process in parallel.",
    )
    parser.add_argument(
        "--randomize",
        type=bool,
        action="store",
        default=False,
        help="Randomly shift/rotate galaxies to prevent repetition, default False.",
    )
    parser.add_argument(
        "--fuzzy_bounds",
        type=bool,
        action="store",
        default=True,
        help="Allow satellites to be collected outside snapshot allowable range, default True.",
    )
    parser.add_argument(
        "--seed",
        type=int,
        action="store",
        default=None,
        help="Seed used if randomization is enabled.",
    )
    parser.add_argument(
        "--key",
        type=str,
        action="store",
        default="Galaxies",
        help="Hdf5 dataset key, default `Galaxies`.",
    )
    parser.add_argument(
        "--numfiles",
        type=int,
        action="store",
        default=1,
        help="Number of output files to split data over",
    )
    ...
    for i, fp in enumerate(files):
        with h5py.File(fp, "r") as f:
            if i == 1:
                cosmology = apcos.LambdaCDM(
                    H0=f.attrs["HubbleParameter"] * 100,
                    Om0=f.attrs["Omega_0"],
    ...
    if Nfiles > 1:
        file_group = [
            tuple(
                [
                    os.path.join(args.indir, "galaxies.S{:d}.{:d}.h5".format(s, j))
    ...
    if args.np > 1:
        with Pool(args.np) as p:
            catalog = list(
                tqdm(
                    p.imap(make_catalog, file_group),
                    total=len(file_group),
    ...
    if args.np > 1:
        with Pool(args.np) as p:
            for _ in tqdm(
                p.imap_unordered(export, out_groups),
                total=args.numfiles,
                desc="Distributing data to {:d} files".format(args.numfiles),
            ):
                pass
    else:
