...
while broken.sum() > 0:
    ...
    broken = (
        iso_trees.loc[iso_trees.loc[satmask]["upid"]].Type == 1
    )  # check if all halos have been fixed this cycle
...
possible_prog_mask = (iso_trees["upid"] == 0) & (
    iso_trees["descid"] != 0
)  # locate all main halos that have a desc.
...
for i, a in enumerate(
    ...
    ][["FileNumber", "rootid"]].values


desc_mask = iso_trees.descid > 0
iso_trees.loc[desc_mask, "desc_mass"] = iso_trees.loc[
    iso_trees.loc[desc_mask, "descid"].values, "mvir"
].values
iso_trees.loc[~desc_mask, "desc_mass"] = iso_trees.loc[
    iso_trees.loc[~desc_mask].index.values, "mvir"
].values
iso_trees["root_mass"] = iso_trees.loc[iso_trees["rootid"].values]["mvir"].values

print("Sorting trees...")
iso_trees.sort_values(
    ["root_mass", "rootid", "scale", "desc_mass", "mmp", "mvir"],
...
for fn in tqdm(np.arange(len(trees_path)), desc="Writing iso trees"):
    ...
    gb.io.emerge_io.write_halo_trees(
        file_path=file_path,
        tree=hout[cout],
        Ntrees=Ntrees,
        Nhalos=Nhalos,
