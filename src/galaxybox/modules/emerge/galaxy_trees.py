...
class GalaxyTrees:
    ...
    def from_file(cls, tree_dir, fields_out=None):
        ...
        f = os.path.join(
            tree_dir, "tree.0.h5"
        )  #! Having this hardcoded is a terrible idea...I'll deal with it later.
        keys = h5py.File(f, "r").attrs.keys()
        values = h5py.File(f, "r").attrs.values()
        attrs = dict(zip(keys, values))
        cosmo = apcos.LambdaCDM(
            H0=attrs["HubbleParameter"] * 100,
            Om0=attrs["Omega_0"],
    ...
    def count_mergers(
        ...
        **kwargs : dict, optional
        ...
        if isinstance(max_z, (float, int)):
            ...
            color_mask = (np.log10(mb["SFR"]) - mb["Stellar_mass"]) < np.log10(
                0.3 / mb["Age"] / self.UnitTime_in_yr
            )
    ...
    def find_mergers(
        ...
        ] = self.trees.loc[minor_progs.index.values][
            [
                "Desc_ID",
                "Halo_mass",
                "Stellar_mass",
        ...
        ] = self.trees.loc[mergers["Desc_ID"].values][
            [
                "Scale",
                "Halo_mass",
                "Stellar_mass",
        ...
        ] = self.trees.loc[mergers["Main_ID"].values][
            ["Halo_mass", "Stellar_mass", "SFR", "Stellar_mass_obs", "SFR_obs", "Type"]
        ].values
        ...
        if desc_mass_correction:
            ...
            def nonbinary_correction(index):
                co_progs_mask = (
                    (mergers["Desc_ID"] == mergers.loc[index].Desc_ID)
                    & (mergers.index != index)
                    & (mergers["tdf"] >= mergers.loc[index].tdf)
                )
                return np.log10(
                    10 ** mergers.loc[index].Desc_mstar
                    - (np.sum(10 ** mergers.loc[co_progs_mask]["Minor_mstar"]))
        ...
        if enforce_mass_threshold:
            low_mass_mask = (
                (mergers["Main_mstar" + add_obs] < self.OutputMassThreshold)
                | (mergers["Minor_mstar" + add_obs] < self.OutputMassThreshold)
                | (mergers["Desc_mstar" + add_obs] < self.OutputMassThreshold)
            )
        ...
        for i, og in enumerate(tqdm(vert)):
            ...
            for j, sa in enumerate(snap_arg):
                ...
                if lean:
                    ...
                    galaxies = self.list(snapshot=snapshots[sa], **kwargs)[
                        [
                            "Scale",
                            "Up_ID",
                            "Halo_radius",
                ...
                if method == "full_width":
                    ...
                    xp, yp, zp = (
                        galaxies["X_pos"].values,
                        galaxies["Y_pos"].values,
                        galaxies["Z_pos"].values,
                    )
                    xv, yv, zv = (
                        galaxies["X_vel"].values,
                    ...
                    (
                        ...
                        xp, yp, zp, xv, yv, zv, xc, yc, zc, vxc, vyc, vzc, np.inf
                    )
    ...
    def merging_time(self, igal_1, igal_2):
        ...
        if self._leaf:
            ...
            raise NotImplementedError(
                "This method not currently availble for trees that haven't been reindexed."
            )
        ...
        **kwargs : dict
        ...
        if ax is None:
            ...
            axins = inset_axes(
                ...
                height="3%",
                loc="lower left",
                bbox_to_anchor=(0.0, 1.02, 1.0, 0.75),
    ...
    def scale_at_massfrac(self, igal, frac, interpolate=False):
        ...
        while prog_mask.sum() > 0:
            log_frac[prog_mask] = (
                self.trees.loc[progid[prog_mask]]["Stellar_mass"].values - m0[prog_mask]
            )
