...
class TreePlotter:
    ...
    def __init__(self, min_scale: int = 0, max_mu: int = 10000, mode: str = "centered"):
        """Initialize the TreePlotter object.

        Parameters
        ----------
        min_scale : int, optional
            The minimum scale factor, by default 0
        max_mu : int, optional
            The maximum mass ratio, by default 10000
        mode : str, optional
            The mode of the tree visualization, by default "centered"
    ...
    def set_pos(self):
        ...
        if self.tree.loc[self.node]["num_prog"] > 0:
            immp = self.tree.loc[
                (self.tree.desc_id == self.node) & (self.tree.mmp == 1)
            ].index.values[0]
    ...
    def render(self):
        ...
        for idx in self.pos[valid_idx].index.values:
            if self.tree.loc[idx].desc_id == 0:
                continue
